...
//	char temp;                // temporary color storage for bgr-rgb conversion. // (USED FOR DEBUGGING IN ORIGINAL CODE)
	...
	fopen_s(&fise, filename, "r");
	if (fise == NULL) {
		printf("File Not Found : %s\n", filename);
		return 0;
	}
     printf("HELLOW WORLD22/n");
	// seek through the bmp header, up to the width/height:
	fseek(fise, 18, SEEK_CUR);
    printf("HELLOW WORLD33/n");
	// read the width
	if (!(image->sizeX = endianReadInt(fise))) {
	...
	// read the height
	if (!(image->sizeY = endianReadInt(fise))) {
	...
	// read the planes
	if (!(planes = endianReadShort(fise))) {
	...
	if ((i = fread(image->data, size, 1, fise)) != 1) {
